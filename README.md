...
│
│   ├── providers/                     managed challenge / managed challenge / managed challenge / 交互挑战 / 交互挑战通用经验
│   └── domains/_example.com/          示例结构; 真实 domain 目录本地保留
│
└── tools/
...
仓库为唯一来源。本机用软链把每个 skill 链回 `~/.claude/skills/`。active skill 数量以 `python3 tools/score_skills.py --repo .` 和 release score 输出为准；当前 release gate 统计为 15 个 active skill：
...
foreach ($n in @('browser-fingerprint-surface-lab','fingerprint-block-reason-diagnostics')) {
  New-Item -ItemType Junction -Path "$env:USERPROFILE\.claude\skills\$n" -Target "E:\SKILLS\oh_my_reverse_skill\7-指纹风控层\$n"
}
```
