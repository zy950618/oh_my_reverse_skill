...
- 每次修改 loop 结构、角色规则、runner、acceptance、metrics 或 freshness gate 后跑 `python3 tools/validate_web_h5_loop_gate.py`、`python3 tools/validate_web_h5_real_execution_gate.py` 和 `python3 tools/ci_gate.py .ci-out`。
- 每次新增或修改公开靶场 evidence 后跑 `python3 tools/validate_public_range_evidence.py public-range-evidence`，并保留本轮 ledger/report 的结构校验结果。
