...
| 公开靶场训练 / public-range-evidence / 用公开靶场进化 SKILLS / 靶场训练进化 SKILLS | public range training / public-range evidence / evolve skills with public ranges | `web-h5-loop-engineering` + `skills-evaluation-governance` | 1 | 按 allowlist 跑公开/本地/官方测试靶场，产出 evidence、负例/边界 eval 和正向门槛判断 | 非 allowlist 真实生产站点 / challenge/WAF 绕过 |
...
## Phase 3 challenge Algorithm Triggers
...
| realistic challenge risk lab / Phase 3.4 lab / runtime parity plus fingerprint plus concurrency | `js-page-runtime-parity` + `browser-fingerprint-surface-lab` + `authorized-target-adapter` | 2/7/1 | Run self-owned localhost lab evidence and keep capability boundaries explicit | Unknown third-party production automation |
