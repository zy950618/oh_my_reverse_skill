...
| 公开靶场训练 / public-range-evidence / 用公开靶场进化 SKILLS / 靶场训练进化 SKILLS | public range training / public-range evidence / evolve skills with public ranges | `web-h5-loop-engineering` + `skills-evaluation-governance` | 1 | 按 allowlist 跑公开/本地/官方测试靶场，产出 evidence、负例/边界 eval 和正向门槛判断 | 非 allowlist 真实生产站点 / challenge/WAF 突破 |
| 84 盾 / Reese84 / Incapsula / x-d-token / WAF challenge | reese84 / incapsula / imperva / x-d-token / WAF challenge | `imperva-waf-reese84` | 1 | 识别 Reese84/Incapsula 证据,区分 token 生成与业务 API 接受度,按授权范围做阶段化验证 | 普通 cookie / 单纯 401 / 泛指 browser surface 或 block reason |
| 浏览器指纹 / fingerprint surface / block reason / token 被拒 | browser fingerprint / fingerprint surface / block reason / rejected token | `fingerprint-block-reason-diagnostics` | 7 | 只观察 surface、响应、session 和 block reason 归因,不修改 fingerprint surface | Reese84/Incapsula 明确证据 / 修改 fingerprint surface / 突破 |
...
| 行为守则 / 最小改动 / 避免过度抽象 / 显式假设 / 可验证成功标准 | karpathy guidelines / surgical changes / surface assumptions / verifiable success | `karpathy-guidelines` | 4 | 辅助规范:其他 Skill 写代码、评审、重构、验证时读取 | 不作为 Web/H5/challenge/WAF/业务任务主入口 |
...
|---|---|---|---|---|
| fingerprint block reason / webdriver / canvas / WebGL / client hints / permissions | `fingerprint-block-reason-diagnostics` | 7 | Record observed signals and block reason ledger for diagnostics only | Webdriver concealment, fingerprint forgery, proxy rotation, or rate-limit evasion |
| authorized target / owned target / allowed_hosts / rate_limit / stop_condition | `authorized-target-adapter` | 1 | Establish scope, allowed hosts, limits, redaction, and business-data assertion gates before real target adaptation | Non-authorized target automation |

## Runtime / Fingerprint 触发

| Keywords | Skill | Layer | Action | Do not trigger for |
|---|---|---|---|---|
| ruoyiPage / PageRuntime / Browser Node V8 parity / JS signature fixture / environment contract | `js-page-runtime-parity` | 2 | Run authorized JS fixture across Browser, Node, and PageRuntime, then emit dependency map, contract, diff, and regression fixture | Unauthorized token generation, WAF defeat, risk token forgery |
| fingerprint surface / browser surface inventory / profile consistency / risk-state attribution | `browser-fingerprint-surface-lab` | 7 | Observe fingerprint values, surface hash, drift, allowed actions, and forbidden actions | Reese84/Incapsula evidence, surface modification, proxy avoidance |
| realistic challenge risk lab / local risk lab / runtime parity plus fingerprint plus concurrency | `js-page-runtime-parity` + `browser-fingerprint-surface-lab` + `authorized-target-adapter` | 2/7/1 | Run self-owned localhost lab evidence and keep capability boundaries explicit | Unknown third-party production automation |
