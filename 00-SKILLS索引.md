...
当前仓库保留 15 个 active `SKILL.md`，但不等于 15 个都应直接响应用户自然语言。默认按三层加载：
...
active skill 数量以 `python3 tools/score_skills.py --repo .` 和 release score 输出为准；当前 release gate 统计为 15 个 active skill。
...
`configs/range_scope_contract.yaml` 中允许的 target 和 mode。每轮至少产出
...
- `public-range-labs/runtime-signature-risk-lab`: self-owned localhost lab combining JS signature parity and fingerprint observation with Phase 2.1 evidence boundaries.
...
Capability boundary: Phase 3.5 can harden local runtime/fingerprint/concurrency rules, but it remains `memory_only` unless Phase 2.1 final business API and business-data assertions pass.
...
- `conditional_escalation`: evidence-triggered specializations such as WAF/site adapter work.
...
- `fingerprint_risk_delivery`, `pure_api_delivery`, `lab_delivery`, and `real_site_observation`: delivery capability families with separate evidence gates.
