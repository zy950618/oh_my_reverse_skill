...
```bash
# 1. 检查 Skills 软链
ls ~/.claude/skills/ | grep -E '(website-|reverse-js|imperva|find-crypto|ast-|env-patch|karpathy|site-api|ai-reverse|skills-evaluation)'
# 应该看到 11 个
...
```bash
# Windows (PowerShell)
foreach ($n in @('website-314-api-delivery','reverse-js-crawler','imperva-waf-reese84','skills-evaluation-governance','find-crypto-entry','ast-deobfuscate','env-patch','ai-reverse-skill-creator','karpathy-guidelines','site-api-adapter','captcha-service-delivery')) {
...
- [ ] `ls ~/.claude/skills/` 看到 11 个 Skill 软链
- [ ] `python --version` ≥ 3.11
