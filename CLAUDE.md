...
- 交付后写 Monitoring Ledger,跟踪 replay、schema、字段、评分、证据过期和历史遗留。
- 不把辱骂性前缀、人格化称谓或情绪化口头禅写入强制输出格式。
...
9. 涉及拒答、人工复核、错误纠正或长期稳定声明时,写对应 ledger 和 monitoring plan
...
- 不在错误发生后只口头说明;必须更新 memory、known-failures、eval 或图谱,并清理错误代码/历史遗留
- 不删除唯一证据;清理前先迁移到 memory、fixture meta 或 failure 样本
- 不把辱骂性前缀、人格化称谓或情绪化口头禅作为交付规范
